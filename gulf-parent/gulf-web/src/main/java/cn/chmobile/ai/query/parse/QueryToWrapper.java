package cn.chmobile.ai.query.parse;
...
import cn.chmobile.ai.query.data.Condition;
import cn.chmobile.ai.query.data.Condition.Filter;
import cn.chmobile.ai.query.data.Condition.Operator;
import cn.chmobile.ai.query.data.Queryable;
import cn.chmobile.ai.utils.ObjectUtils;
...
public class QueryToWrapper<T> {
	...
	public void parseSort(Wrapper<T> wrapper, Queryable queryable) {
		cn.chmobile.ai.query.data.Sort sort = queryable.getPageable().getSort();
		if (sort != null) {
			for (cn.chmobile.ai.query.data.Sort.Order order : sort) {
				if (order.getDirection() == cn.chmobile.ai.query.data.Sort.Direction.DESC) {
					wrapper.orderBy(order.getProperty(), false);
				} else if (order.getDirection() == cn.chmobile.ai.query.data.Sort.Direction.ASC) {
