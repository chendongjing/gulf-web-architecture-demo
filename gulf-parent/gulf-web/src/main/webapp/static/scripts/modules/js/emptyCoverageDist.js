...
function Map_Depthcoverage(options) {
	...
	return {
		'show':function(renderId, params) {
			...
			var myIcon = new BMap.Icon(projectName+"/static/scripts/modules/images/position.png", new BMap.Size(23, 25), {  
                offset: new BMap.Size(10, 25), // 指定定位位置  
                imageOffset: new BMap.Size(0, 0 - 0 * 25) // 设置图片偏移  
            }); 
			var position = new BMap.Marker(point, {icon:myIcon});   // 创建标注
			var infoWindow = new BMap.InfoWindow("定位位置：<br>经度:" + point.lng+ "<br>纬度:" + point.lat, opts);  // 创建信息窗口对象 
			position.addEventListener("click", function(){          
				map.openInfoWindow(infoWindow,point); //开启信息窗口
			});
			//切换后去除所有覆盖物
	    	map.clearOverlays();
			map.addOverlay(position);  //添加位置覆盖物
